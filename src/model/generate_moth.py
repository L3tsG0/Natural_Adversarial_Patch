...
nc: Final[int] = 3 # チャンネル数．カラー画像ならRGBの3

nz: Final[int] = 100 # 潜在変数の数


ngf: Final[int] = 64 # Size of feature maps in generator


ndf: Final[int] = 64# Size of feature maps in discriminator
...
interface = None
def set_interface(_arg: AEengine_interface):
    global interface
    interface = _arg
...
def main(dataset_path: str, learning_cycles: int):
    ...
    # setting of hyper-params for GAN 

    dataroot: Path = Path(dataset_path)  # Root directory for dataset

    # datarootの下にclass1なるサブディレクトリを作成→その中に学習させたい(真似したい)画像を入れる
    # 画像はできれば背景なしが良い．パッチなので． see leedbutterfly/dataset/class1

    workers: Final[int] = 2  # Number of workers for dataloader


    # remark.この値は大きすぎるとモデルによってはmemory Errorを出す．
    # UI側で指定できるようにすると良いかもしれない
    batch_size: int = 1024 # Batch size during training
    ...
    plt.ylabel("Loss")
    plt.legend()
    plt.savefig("learning_result.png",dpi = 200)
    ...
    #img_list→[[1エポック目で生成された画像群],[],....[Nエポック目で生成された画像群]]
